...
public:
    ...
    void start();
    void deliver(const string& line);
...
public:
    ...
    void join(shared_ptr<ClientSession> s) {
        auto ip = s->remote_ip();
    ...
    void leave(shared_ptr<ClientSession> s) {
        sessions.erase(s);
        if (!s->name.empty()) {
            online.erase(s->name);
        }
    ...
    void on_line(shared_ptr<ClientSession> s, const string& raw) {
        string line = raw;
        while (!line.empty() && (line.back()=='\n' || line.back()=='\r')) line.pop_back();

        if (line.rfind("#me", 0) == 0) {
            string newname = trim_after(line, "#me");
        ...
        if (line.rfind("#block", 0) == 0) {
            string who = trim_after(line, "#block");
        ...
        if (line.rfind("#unblock", 0) == 0) {
            string who = trim_after(line, "#unblock");
        ...
        if (line.rfind("#fav", 0) == 0) {
            string who = trim_after(line, "#fav");
        ...
        if (line.rfind("#unfav", 0) == 0) {
            string who = trim_after(line, "#unfav");
        ...
        if (line.rfind("#massdm", 0) == 0) {
            string text = trim_after(line, "#massdm");
        ...
        if (line.rfind("@", 0) == 0) {
            auto sp = line.find(' ');
            string to = (sp == string::npos) ? line.substr(1) : line.substr(1, sp-1);
            string text = (sp == string::npos) ? "" : line.substr(sp+1);
    ...
    static string trim_after(const string& line, const string& cmd) {
        if (line.size() <= cmd.size()) return "";
        string rest = line.substr(cmd.size());
...
private:
    void do_accept() {
        acceptor_.async_accept(
            [this](const boost::system::error_code& ec, tcp::socket socket) {
                if (!ec) {
                    auto session = make_shared<ClientSession>(std::move(socket), *this);
                    join(session);
                    session->start();
                }
                do_accept();
    ...
    void broadcast_public(const string& from, const string& text) {
        for (auto& [name, sess] : online) {
    ...
    bool is_blocked(const string& receiver, const string& sender) {
        auto it = blacklist.find(receiver);
        if (it == blacklist.end()) return false;
        return it->second.count(sender) > 0;
    }

    void deliver_offline_inbox_if_any(shared_ptr<ClientSession> s) {
    ...
    unordered_set<shared_ptr<ClientSession>> sessions;

public:
    unordered_map<string, shared_ptr<ClientSession>> online;
    unordered_map<string, string> last_name_by_ip;
    unordered_map<string, unordered_set<string>> blacklist;
    unordered_map<string, unordered_set<string>> favorites;
    unordered_map<string, deque<pair<string,string>>> offline_inbox;
...
void ClientSession::deliver(const string& line) {
...
void ClientSession::on_read(const boost::system::error_code& ec, size_t) {
    if (ec) {
